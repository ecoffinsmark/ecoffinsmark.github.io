...
function AnimationItem(obj, seq){
	...
	this.play = function(){
		...
		if(this.$element.parents('.pageItem').hasClass('hidden')) { this.next() };
		return this;
		...
		mso.each(function(index,elem) {
			if(elem.hasOwnProperty('loopcount')) {
	 		...
	 		if(elem.crossfade > 0) {
				var el = $(elem).removeClass('hidden');
				var last = el.children('.state.active').removeClass('active').addClass('transition').show().fadeOut(elem.crossfade, function(){$(this).removeClass('transition')});
				el.children('.state').first().addClass('active').hide().fadeIn(elem.crossfade, function(e) { last.hide();}).trigger('newState');
		...
		mso.each(function(index,elem) {
			if(elem.hasOwnProperty('loopcount')) {
	 		...
	 		if(elem.crossfade > 0) {
				var el = $(elem).removeClass('hidden');
				var last = el.children('.state.active').removeClass('active').addClass('transition').show().fadeOut(elem.crossfade,function(){$(this).removeClass('transition')});
				el.children('.state').last().addClass('active').hide().fadeIn(elem.crossfade,function(e) { last.hide();}).trigger('newState');
...
function addNavProps(){
	if(nav.numPages === undefined) nav.numPages=25;
...
$(window).load(function(){
	...
	if(sliderSettings.hashTags && !sliderSettings.disableBrowserNav){ $(window).on('hashchange', function(e){ nav.to(api.gotoHash()); }); }		
